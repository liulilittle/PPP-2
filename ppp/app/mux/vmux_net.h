...
#pragma pack(pop)
    ...
    public:
        vmux_net(const ContextPtr& context, const StrandPtr strand, uint16_t max_connections, bool server_mode) noexcept;
        ~vmux_net() noexcept;
    ...
    private:
        struct {
            ...
            std::atomic<int>                                                        ftt_               = false;
            std::atomic<int>                                                        server_or_client_  = false;
            std::atomic<int>                                                        established_       = false;
        }                                                                           base_;

        struct {
            std::atomic<uint16_t>                                                   max_connections    = 0;
